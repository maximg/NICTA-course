...
instance Functor (State s) where
  ...
  (<$>) f x =
      ...
      in State { runState = applyFst f . (runState x) }
...
isHappy x =
  let
    digits x' = P.map DC.digitToInt $ show x'
    square x' = x' * x'
    sumSq x' = toInteger $ P.foldr ((+) . square) 0 $ digits x'
    p x' = (\s -> (const $ pure (x' == 1 || S.member x' s)) =<< put (S.insert x' s)) =<< get
    res = fst $ runState (findM p $ produce sumSq x) S.empty
  in contains 1 res
